...
# Bot token
BOT_TOKEN = '7047464931:AAF72fboI32pakXmZ_CdUTbPxarANB6X8tQ'  # Replace with your bot token
...
async def bgmi(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # If no arguments are provided, inform the user about required inputs
    if len(context.args) == 0:
        await update.message.reply_text(
            "📋 *No attack details provided.*\n"
            "✅ *Usage:* /bgmi <ip> <port> <duration>\n"
            "💡 Or type /bgmi to check attack status.",
            parse_mode="Markdown",
        )
        return
